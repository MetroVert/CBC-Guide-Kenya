...
const SUBJECTS = [
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="10"/><path d="m15 9-6 6M9 9h.01M15 15h.01"/>
      </svg>
    ),
    color: "purple",
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 22V12"/><path d="M5 12H2a10 10 0 0 0 20 0h-3"/><path d="M8 5.2C5.8 6 4 8.3 4 11c0 .9.2 1.8.5 2.6"/><path d="M19.5 13.6A8 8 0 0 0 20 11c0-2.7-1.8-5-4-5.8"/><path d="M15.5 4.8a6 6 0 0 0-7 0"/>
      </svg>
    ),
    color: "green",
...
const COLOR_MAP: Record<string, { bg: string; icon: string; border: string }> = {
  blue:   { bg: "hsl(210, 85%, 40%, 0.08)", icon: "hsl(210, 85%, 40%)", border: "hsl(210, 85%, 40%, 0.2)" },
  green:  { bg: "hsl(152, 64%, 35%, 0.08)", icon: "hsl(152, 64%, 33%)", border: "hsl(152, 64%, 35%, 0.2)" },
  teal:   { bg: "hsl(172, 60%, 35%, 0.08)", icon: "hsl(172, 60%, 33%)", border: "hsl(172, 60%, 35%, 0.2)" },
  purple: { bg: "hsl(265, 55%, 48%, 0.08)", icon: "hsl(265, 55%, 45%)", border: "hsl(265, 55%, 48%, 0.2)" },
  orange: { bg: "hsl(25, 85%, 50%, 0.08)",  icon: "hsl(25, 85%, 47%)",  border: "hsl(25, 85%, 50%, 0.2)" },
  pink:   { bg: "hsl(340, 65%, 48%, 0.08)", icon: "hsl(340, 65%, 45%)", border: "hsl(340, 65%, 48%, 0.2)" },
  gold:   { bg: "hsl(42, 90%, 45%, 0.08)",  icon: "hsl(42, 90%, 40%)",  border: "hsl(42, 90%, 45%, 0.2)" },
};
      ...
      <nav className="breadcrumb-bar" aria-label="Breadcrumb">
        <div className="breadcrumb-inner">
          <ol className="breadcrumb-list">
            ...
            <li>
              <Link href="/grades/grade-8" className="breadcrumb-link breadcrumb-link--active" aria-current="page">
                Grade 8
              </Link>
            </li>
          </ol>
      ...
      <section className="grade-page-header">
        <div className="grade-page-header-inner">
          ...
          <p className="grade-page-intro">
            Welcome to your Grade 8 hub! This is where Junior Secondary gets serious — and exciting.
            You're building on everything you learned in primary, diving deeper into science, language,
            and technology. Pick a subject below to access organised notes, practice questions, and
            learning activities crafted to match the CBC curriculum. Every resource here is mapped to
            your learning outcomes, so you study smarter, not harder.
          </p>

          {/* Search bar */}
          <div className="subject-search-wrap">
            <div className="subject-search">
              <svg className="search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <div className="subjects-inner">
          {filtered.length > 0 ? (
            ...
            <div className="subjects-empty">
              <p>No subjects match "<strong>{query}</strong>". Try a different search.</p>
              <button className="subjects-empty-reset" onClick={() => setQuery("")}>Clear search</button>
            </div>
          )}
        </div>
      </section>
